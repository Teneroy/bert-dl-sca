...
from mlp_power_trace import CustomDataset
...
def load_dpa(dpa_database_file, nrows=100000):    
    ...
    print("Data Type raw traces",type(X_profiling[0][1]))
    print(X_profiling)

    Y_profiling=dfL=df["Label"]
    print(Y_profiling.dtype)
...
def full_ranks(model, dataset, metadata_plaintext,metadata_key, min_trace_idx, max_trace_idx, rank_step):
    ...
    test_dataset = CustomDataset(dataset, pd.DataFrame(np.zeros(len(dataset))))  # We only need traces for prediction, labels can be dummy
